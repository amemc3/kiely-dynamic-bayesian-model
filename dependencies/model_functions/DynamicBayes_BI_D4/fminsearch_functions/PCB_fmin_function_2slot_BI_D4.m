% PCB_bayes_function_2slot_BI_D4
% Xmax = x1 for each trial
% Xchoice = distro for the chosen slot on each trial
function [sum_likely, Xmax, Xchoice] = PCB_fmin_function_2slot_BI_D4(pars, data, upperbound, linlog)
...
elseif pars(3)<1/2 || pars(3)>1
    likely=ones(1,n_blocks)*1000;
elseif pars(4)<1/2 || pars(4)>1
    likely=ones(1,n_blocks)*1000;
else
    ...
    for blocki=1:n_blocks
        ...
        for triali=start_trial:length(real_choices)
            ...
            if  real_choices(triali-1)>0
                ...
                slotp{blocki}(triali,:)=(prob + ones(1,2)/2)/2;
            end

            distro=slotp{blocki}(triali,:);
            % 2025-04-07: added for confidence analysis
            Xmax(blocki,triali)=(max(distro)-1/2)*2;
            if real_choices(triali) ~= 0
                Xchoice(blocki,triali) = distro(real_choices(triali));
            else
                Xchoice(blocki,triali) =(max(distro)-1/2)*2;
            end
