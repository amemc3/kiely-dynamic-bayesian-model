% PCB_bayes_function_3fish_BI_D4
function sum_likely = PCB_fmin_function_3fish_BI_D4_SMx(pars, data, upperbound, linlog, tau)
...
elseif pars(4)<1/3 || pars(4)>1
    ...
    for blocki=1:n_blocks
        ...
        for triali=start_trial:length(fish_disp)
            ...
            % % Softmax 2.0
            distro= exp(pondp{blocki}(triali,:)/tau)/sum(exp(pondp{blocki}(triali,:)/tau));
