%% PCB_Model_3Slot_BH_1
% Adapted from PCB_Model_3slot_BI_1 and fiore heuristic
% This script runs a heuristic model on the 3 slot task.
% It generates a .mat file (3slot_BH_1_group.mat) containing the 
...
function PCB_Model_3slot_H_1(data_folder, root_folder, day, group)
            ...
            for triali=start_trial:length(outcomes)
                ...
                if real_choices(triali-1) == 1
                    if outcomes(triali-1) == 100 % if win
                        ...
                        slotpN100{blocki}(triali,2) = prob(2)-h/2;
                        slotpN100{blocki}(triali,3) = prob(3)-h/2;
                ...
                elseif real_choices(triali-1) == 2
                    if outcomes(triali-1) == 100 % if win
                        slotpN100{blocki}(triali,1) = prob(1)-h/2;
                ...
                elseif real_choices(triali-1) == 3
                    if outcomes(triali-1) == 100 % if win
                        slotpN100{blocki}(triali,1) = prob(1)-h/2;
                        slotpN100{blocki}(triali,2) = prob(2)-h/2;
                        slotpN100{blocki}(triali,3) = prob(3)+h;
                    elseif outcomes(triali-1) == 10 || outcomes(triali-1) == 0 % if loss
                        slotpN100{blocki}(triali,1) = prob(1)+h/2;
                        slotpN100{blocki}(triali,2) = prob(2)+h/2;
                        slotpN100{blocki}(triali,3) = prob(3)-h;
...
cd (root_folder)
filename = [day '_3slot_H_1_' group '.mat'];
