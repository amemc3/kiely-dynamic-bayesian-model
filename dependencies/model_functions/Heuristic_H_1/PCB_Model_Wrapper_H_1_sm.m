%% PCB_Model_Wrapper_H_1
% this is the wrapper function which runs the 
% heuristic model (H_1) for a given task

% data_folder = simulated behavior on a task for a given model
% root_folder = where the modeled data will be stored
% group = The name of the data-generating model for the data folder

% uses softmax

function PCB_Model_Wrapper_H_1_sm(data_folder, root_folder, day, group, task)
...
for agent=1:n_subjects
    ...
    funcName = ['PCB_fmin_function_', task, '_H_1_SMx'];
...
cd(root_folder)
filename = [day '_' task '_H_1_sm_' group '.mat'];
