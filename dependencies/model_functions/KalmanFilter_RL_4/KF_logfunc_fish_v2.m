function total_log = KF_logfunc_fish_v2(v, s, tau, data)
...
for iB = 1:nBlocks
    ...
    expected_reward(1,:) = (ones(1,3)/3)*100;
    ...
    for iT = 1:nTrials
        ...
        if iT ~= 1
            ...
            % Extract reward
            if data{iB,1}(iT,5) == user_choice
                reward = 100;
            elseif data{iB,1}(iT, 5) ~= user_choice
                reward = 0;
            end
        end
        ...
        if iT ~= 1
            ...
            for i = 1:3
                if A(i) > 0
                    % Weighted variance
                    sigma(i) = ((1^2)*E1(i)+(2^2)*E2(i)) - ((1)*E1(i)+(2)*E2(i))^2;
                else
                    sigma(i) = .5;
                end
                % Sigma bounds
