%% PCB_Model_Wrapper_RL_KF1
% this is the wrapper function which runs the 
% kalman filter RL model (RL_KF1) for a given task

function PCB_Model_Wrapper_RL_KF1(data_folder, root_folder, day, group, task)
...
upperbound=100; % max values for v, s, and tau      
lowerbound = 0; % min values 
...
for agent=1:n_subjects
    ...
    funcName = ['PCB_fmin_function_', task, '_RL_KF1'];
    
    % Create function handle dynamically
    fminsearch_function = str2func(funcName);
    
    % replaced with AEM version of bayes_function
    [lambdalin,fvalin] = fminsearch(@(pars) fminsearch_function(pars, DATA.data, upperbound, lowerbound, 'lin'), 1);%[best_set(agent,1)]);
    [lambdalog,fvalog] = fminsearch(@(pars) fminsearch_function(pars, DATA.data, upperbound, lowerbound, 'log'), 1);%[best_set(agent,3)]);
...
cd(root_folder)
filename = [day '_' task '_RL_KF1_' group '.mat'];
