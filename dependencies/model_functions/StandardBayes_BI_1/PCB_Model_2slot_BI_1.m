%% PCB_Model_2Slot_BI_1
% Adapted from Fiore bayes_main (3fish version)
% This script runs a standard bayesian model on the 2 slot task.
% It generates a .mat file (2slot_BI_1_group.mat) containing the 
...
function PCB_Model_2slot_BI_1(data_folder, root_folder, day, group)
...
for agent=1:n_subjects
    ...
    for iter=1:n_iter
        ...
        s2=m1;
        % win matrix
        matW= [0 1; 1 0]*s1+eye(2)*m1;
        % loss matrix
        matL= [0 1; 1 0]*s2+eye(2)*m2;
        ...
        for blocki=1:n_blocks
            ...
            for triali=start_trial:length(outcomes)
                ...
                if  real_choices(triali-1)>0
                    if outcomes(triali-1)==100 % if won
                        % denominator: sum(prior x likelihood)
                        denN100=sum(prob.*matW(real_choices(triali-1),:));
                        % posterior: prior x likelihood / denominator
                        slotpN100{blocki}(triali,:)=(matW(real_choices(triali-1),:).*slotpN100{blocki}(triali-1,:))/denN100;
                    elseif  outcomes(triali-1)==0 % if loss
                        denN100=sum(prob.*matL(real_choices(triali-1),:));
                        slotpN100{blocki}(triali,:)=(matL(real_choices(triali-1),:).*slotpN100{blocki}(triali-1,:))/denN100;
                    end
                else
...
cd (root_folder)
filename = [day '_2slot_BI_1_' group '.mat'];
