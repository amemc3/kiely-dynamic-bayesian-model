%% PCB_Model_3Slot_BI_1
% Adapted from Fiore bayes_main (3fish version)
% This script runs a standard bayesian model on the 3 slot task.
% It generates a .mat file (3slot_BI_1_group.mat) containing the 
...
function PCB_Model_3slot_BI_1(data_folder, root_folder, day, group)
    ...
    for iter=1:n_iter
        ...
        for blocki=1:n_blocks
            ...
            for triali=start_trial:length(outcomes)
                ...
                % Bayesian update: Normal and Bimodal
                if  real_choices(triali-1)>0
                    if outcomes(triali-1)==100
                        % denominator: sum(prior x likelihood)
                        denN100=sum(prob.*slotupdN100(real_choices(triali-1),:));
                        % posterior: prior x likelihood / denominator
                        slotpN100{blocki}(triali,:)=(slotupdN100(real_choices(triali-1),:).*slotpN100{blocki}(triali-1,:))/denN100;
                    elseif  outcomes(triali-1)==10
                        denN100=sum(prob.*slotupdB100(real_choices(triali-1),:));
                        slotpN100{blocki}(triali,:)=(slotupdB100(real_choices(triali-1),:).*slotpN100{blocki}(triali-1,:))/denN100;
                    elseif  outcomes(triali-1)==0
                        denN100=sum(prob.*slotupdB100(real_choices(triali-1),:));
                        slotpN100{blocki}(triali,:)=(slotupdB100(real_choices(triali-1),:).*slotpN100{blocki}(triali-1,:))/denN100;
                    end
                else
                    slotpN100{blocki}(triali,:)=(prob + ones(1,3)/3)/2;
                end
...
cd (root_folder)
filename = [day '_3slot_BI_1_' group '.mat'];
