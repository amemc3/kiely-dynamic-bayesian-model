%% PCB_Model_Wrapper_RL_2
% this is the wrapper function which runs the 
% standard RL model (RL_2) for a given task
% fits tau as a free parameter

function PCB_Model_Wrapper_RL_2(data_folder, root_folder, day, group, task)
...
for agent=1:n_subjects
    ...
    funcName = ['PCB_fmin_function_', task, '_RL_2'];
    
    % Create function handle dynamically
    fminsearch_function = str2func(funcName);
    
    % replaced with AEM version of bayes_function
    [lambdalin,fvalin] = fminsearch(@(pars) fminsearch_function(pars, DATA.data, mxb, 'lin'), [0.75 20]);
    [lambdalog,fvalog] = fminsearch(@(pars) fminsearch_function(pars, DATA.data, mxb, 'log'), [0.75 20]);
...
cd(root_folder)
filename = [day '_' task '_RL_2_' group '.mat'];
